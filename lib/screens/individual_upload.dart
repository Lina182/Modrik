...
import 'individual_report_screen.dart'; // 🔥 أضفنا هذا
...
class _IndividualUploadScreenState extends State<IndividualUploadScreen> {
  ...
  void showLoading(BuildContext context) {
    showDialog(
      context: context,
      barrierDismissible: false, // ما يقفل لو ضغط برا
      barrierColor: Colors.black.withOpacity(0.3), // شفافية
      builder: (_) {
        return Center(
          child: Container(
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: selectedFileName == null
                              ? null
                              : () async {
                                  showLoading(context); // 👈 يطلع اللودينق

                                  await Future.delayed(
                                    const Duration(seconds: 3),
                                  ); // مؤقت

                                  Navigator.pop(context); // 👈 يقفل اللودينق
